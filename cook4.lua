...
function scene:create( event )
    ...
    local function delAll() -- 조리대 위 음식 모두 삭제
        ...
        gameUI[3].alpha = 0
        for i = 1, 4, 1 do kimbap[i].alpha = 0 end
    ...
    local function makeKimbap()
        ...
        for i = 1, 8, 1 do -- 일단 김밥을 말면 재료를 놓지도, 재료소진으로 인한 가격소진도 없음
            IG[i]:removeEventListener("tap", putIG) 
            IG[i]:removeEventListener("tap", calcIG)
        end 

        -- 꼬마김밥: 김, 밥, 달걀, 단무지, 햄
        if (usedIG[1].alpha == 1 and usedIG[2].alpha == 1 and usedIG[3].alpha == 1 and usedIG[4].alpha == 1 and usedIG[5].alpha == 1 and usedIG[6].alpha == 0 and usedIG[7].alpha == 0 and usedIG[8].alpha == 0) then
            for i = 1, 8, 1 do usedIG[i].alpha = 0 end
            kimbap[1].alpha = 1
    ...
    for i = 1, 8, 1 do IG[i]:addEventListener("tap", putIG) end
    for i = 1, 8, 1 do IG[i]:addEventListener("tap", calcIG) end
    ...
    for i = 1, 8, 1 do sceneGroup:insert(IG[i]) end
    sceneGroup:insert(usedIG[2])
    sceneGroup:insert(usedIG[1])
    for i = 3, 8, 1 do sceneGroup:insert(usedIG[i]) end
