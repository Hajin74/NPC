...
local counterCh = audio.play( counter, { channel=9, loops=0, fadein=4000 })
...
local orderUI = {} -- 1:주문말풍선, 2:주문수락, 3:주문거절, 4:꼬마김밥주문, 5:김치김밥주문, 6: 참치김밥주문, 7: 새우김밥주문, 8: 불고기김밥주문
...
function scene:create( event )
	...
	background[1] = display.newImageRect("img/company.jpg", 1250, 460)
	...
	person[1] = display.newImageRect("img/cperson1.png", 220, 260)
	person[2] = display.newImageRect("img/cperson2.png", 220, 260)
	person[3] = display.newImageRect("img/cperson3.png", 220, 260)
	for i = 1, 3, 1 do 
	...
	kimbap[5] = display.newImageRect("img/kimbap5.png", 300, 100)
	for i = 1, 5, 1 do
	...
	function calcKimbap(event)
		...
		elseif orderUI[7].alpha == 1 and event.target == kimbap[4] then -- 4:새우김밥일 때
			...
			calcBank(3500)
			leftUI[4].text = string.format("%d원", money)
		elseif orderUI[8].alpha == 1 and event.target == kimbap[5] then -- 4:불고기김밥일 때
			playClacKimbap()
			money = money + 4000
			calcBank(4000)
			leftUI[4].text = string.format("%d원", money)
	...
	local function order() -- 주문창, 주문내용, 수락/거절 버튼 뜸
		...
		orderUI[math.random(4,8)].alpha = 1
	end
	...
	for i = 1, 5, 1 do kimbap[i]:addEventListener("tap", calcKimbap) end
	for i = 1, 5, 1 do kimbap[i]:addEventListener("tap", delAll) end
	...
	for i = 1, 8, 1 do sceneGroup:insert(orderUI[i]) end
end
