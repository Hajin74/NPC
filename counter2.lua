...
local counterCh = audio.play( counter, { channel=6, loops=0, fadein=4000 })
...
local person = {} -- 초등학생1, 초등학생2, 초등학생3, 중딩1, 중딩2, 중딩3
local kimbap = {}

function scene:create( event )
	local sceneGroup = self.view

	background[1] = display.newImageRect("img/middleschool.jpg", 1250, 460)
    ...
    -- 김밥 주문 종류
	orderUI[4] = display.newText("간단하게 꼬마김밥 렛츠고!", 850, 220, "굴림")
	orderUI[4].size = 30
	orderUI[4]:setFillColor(0)
	orderUI[5] = display.newText("매운게 땡기는 날이야.\n김치김밥 주세요!", 850, 220, "굴림")
	orderUI[5].size = 30
    orderUI[5]:setFillColor(0)
	for i = 1, 5, 1 do orderUI[i].alpha = 0 end

	person[1] = display.newImageRect("img/mperson1.png", 210, 250)
	person[2] = display.newImageRect("img/mperson2.png", 210, 250)
	person[3] = display.newImageRect("img/mperson3.png", 210, 250)
	for i = 1, 3, 1 do 
	...
	local function order() -- 주문창, 주문내용, 수락/거절 버튼 뜸
		...
		orderUI[math.random(4,5)].alpha = 1
	end
	...
	for i = 1, 5, 1 do sceneGroup:insert(orderUI[i]) end
end
